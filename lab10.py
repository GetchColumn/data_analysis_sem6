...
def gaussian_kernel(x1, x2, sigma):
    # вычисляем квадрат расстояния между x1 и x2
    diff = x1 - x2
    sq_diff = np.dot(diff, diff)  # квадрат евклидова расстояния
    # вычисляем гауссово ядро
    sim = np.exp(-sq_diff / (2 * sigma**2))
    return sim


# Подбор параметров для SVM-модели
def adjust_params(X, y, Xval, yval):
    # Задаем значения для C и sigma
    C_values = [0.1, 0.5, 1.0, 5.0, 100.0, 1000.0]
    sigma_values = [0.05, 0.1, 0.3, 0.5, 1.0, 3.0]
    best_C = C_values[0]
    best_sigma = sigma_values[0]
    best_accuracy = 0
    # Перебор всех значений C и sigma
    for C in C_values:
        for sigma in sigma_values:
            ...
            # Проверка, является ли текущая точность наилучшей
